...
router.get('/getdraft', async (req, res) => {
  const getAllQ = 'SELECT draftreports.id,draftreports.category,draftreports.event,draftreports.lga,draftreports.ward,draftreports.date,draftreports.place,users.first_name,users.last_name,users.type FROM draftreports left join users on draftreports.sid=users.id';
  try {
    // const { rows } = qr.query(getAllQ);
    const { rows } = await db.query(getAllQ);
    return res.status(201).send(rows);
